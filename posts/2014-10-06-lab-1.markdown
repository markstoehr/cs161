...
Remember SVN will only track a file `foo` if you call `svn add food` and it
SVN will only save changes if you run something like `svn commit -m "worked on food"` after making modifications to foo.  You can confirm that a file `foo` is in the repository by calling

```bash
svn info foo
```

or by visiting the Phoenixforge website:

```bash
https://phoenixforge.cs.uchicago.edu/projects/[cnetid]-cs161-aut-14/repository/show/
```
where `[cnetid]` is replaced with your cnetid.
...
<center>$$\omega_{n,j}=e^{-\frac{2\pi ij}{n}}=\cos(2\pi\frac{j}{n})- i\sin(2\pi \frac{j}{n}).$$</center>
Note that in Haskell the complex number $a+bi$ is entered in as
$\texttt{a :+ b}$. So that the complex number $i$ is written $\texttt{0.0 :+ 1.0}$.
